        ...
        match socket.recv_from(&mut buf).await {
            Ok((len, addr)) => {
                ...
                if let Ok(cmd) = bincode::deserialize::<RobotCommand>(&buf[..len]) {
                    println!("Received command: {cmd:?} from {addr}");
                    ...
                    let telemetry = state_guard
                        .last_telemetry
                        .clone()
                        .unwrap_or(RobotTelemetry {
                            left_actual: 0,
                            right_actual: 0,
                            battery_mv: 0,
...
pub async fn serial_service(state: Arc<RwLock<RobotState>>) -> Result<()> {
    ...
    let mut last_command_time =
        tokio::time::Instant::now() - tokio::time::Duration::from_millis(25);
                ...
                while let Some(pos) = rx_buffer.iter().position(|&b| b == b'\n') {
                    ...
                    if parts.len() >= 4 && parts[0] == "TEL" {
                        ...
                        if let (
                            ...
                            println!(
                                "Odometry: left_ticks={left_ticks}, right_ticks={right_ticks}, left_vel={left_vel}, right_vel={right_vel}"
                            );
...
pub async fn http_service(state: Arc<RwLock<RobotState>>) -> Result<()> {
    ...
    let debug = warp::path("debug").and(state_filter.clone()).and_then(
    ...
    let odometry = warp::path("odometry").and(state_filter.clone()).and_then(
        |state: Arc<RwLock<RobotState>>| async move {
            let s = state.read().await;
